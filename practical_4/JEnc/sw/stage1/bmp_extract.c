...
//#include "../jpeg_addr.h"  /* No LEDs in this design */
//#include <io.h>             /* Only needed for IOWR_8DIRECT (LED) — not used */
#include <unistd.h>
...
int bmp_extract(char *file_name, Bmp_data *pic_data) {
	...
	//IOWR_8DIRECT(LED_BASE,OFFSET,3);  /* No LEDs */
	...
	//IOWR_8DIRECT(LED_BASE,OFFSET,7);  /* No LEDs */
	fclose(bmp_file);
