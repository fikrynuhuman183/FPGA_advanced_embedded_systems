 ...
 * from a FIFO CSR base and prints them.  "tag" is a short label.
 * --------------------------------------------------------------------- */
                            ...
                            UINT32 image_height)
{
    ...
    printf("S1: total MCUs = %lu, quality = %lu\n",
           (unsigned long)num_mcus, (unsigned long)quality_factor);

    /* ---- Send parameters to Stage 4 (was SEND4) ---- */
    printf("S1: Sending params to Stage 4...\n");
    log_fifo_csr("1to4_IN", FIFO_1TO4_PARAM_IN_CSR_BASE);
    fifo_write_32(FIFO_1TO4_PARAM_IN_BASE, FIFO_1TO4_PARAM_IN_CSR_BASE,
                  num_mcus);
    fifo_write_32(FIFO_1TO4_PARAM_IN_BASE, FIFO_1TO4_PARAM_IN_CSR_BASE,
                  quality_factor);
    printf("S1: Params sent to Stage 4.\n");
    log_fifo_csr("1to4_IN", FIFO_1TO4_PARAM_IN_CSR_BASE);

    /* ---- Send parameters to Stage 5 (was SEND5) ---- */
    printf("S1: Sending params to Stage 5...\n");
    log_fifo_csr("1to5_IN", FIFO_1TO5_PARAM_IN_CSR_BASE);
    ...
    fifo_write_32(FIFO_1TO5_PARAM_IN_BASE, FIFO_1TO5_PARAM_IN_CSR_BASE,
                  num_mcus);
    printf("S1: Params sent to Stage 5.\n");
    log_fifo_csr("1to5_IN", FIFO_1TO5_PARAM_IN_CSR_BASE);
    ...
    for (i = 1; i <= jpeg_encoder_structure->vertical_mcus; i++)
    {
        ...
        for (j = 1; j <= jpeg_encoder_structure->horizontal_mcus; j++)
        {
            ...
            /* Log FIFO status BEFORE pushing this MCU */
            {
                UINT32 mcu_idx = (UINT32)(i - 1) *
                    (UINT32)jpeg_encoder_structure->horizontal_mcus + (UINT32)j;
                printf("S1: MCU #%lu  -- pre-write FIFO status:\n",
                       (unsigned long)mcu_idx);
                log_fifo_csr("Y_IN ",  FIFO_1TO2_Y_IN_CSR_BASE);
                log_fifo_csr("Cb_IN",  FIFO_1TO2_CB_IN_CSR_BASE);
                log_fifo_csr("Cr_IN",  FIFO_1TO2_CR_IN_CSR_BASE);
            }

            /* Convert this MCU block and push via FIFOs */
            read_format(jpeg_encoder_structure, input_ptr);

            /* Log FIFO status AFTER pushing this MCU */
            {
                UINT32 mcu_idx = (UINT32)(i - 1) *
                    (UINT32)jpeg_encoder_structure->horizontal_mcus + (UINT32)j;
                printf("S1: MCU #%lu  -- post-write FIFO status:\n",
                       (unsigned long)mcu_idx);
                log_fifo_csr("Y_IN ",  FIFO_1TO2_Y_IN_CSR_BASE);
                log_fifo_csr("Cb_IN",  FIFO_1TO2_CB_IN_CSR_BASE);
                log_fifo_csr("Cr_IN",  FIFO_1TO2_CR_IN_CSR_BASE);
                printf("S1: MCU #%lu  pushed (192 samples)\n\n",
                       (unsigned long)mcu_idx);
            }
...
int main(void)
{
    ...
    printf("  FIFO_1TO2_Y_IN_BASE       = 0x%08x\n", (unsigned)FIFO_1TO2_Y_IN_BASE);
    printf("  FIFO_1TO2_Y_IN_CSR_BASE   = 0x%08x\n", (unsigned)FIFO_1TO2_Y_IN_CSR_BASE);
    printf("  FIFO_1TO2_CB_IN_BASE      = 0x%08x\n", (unsigned)FIFO_1TO2_CB_IN_BASE);
    printf("  FIFO_1TO2_CB_IN_CSR_BASE  = 0x%08x\n", (unsigned)FIFO_1TO2_CB_IN_CSR_BASE);
    printf("  FIFO_1TO2_CR_IN_BASE      = 0x%08x\n", (unsigned)FIFO_1TO2_CR_IN_BASE);
    printf("  FIFO_1TO2_CR_IN_CSR_BASE  = 0x%08x\n", (unsigned)FIFO_1TO2_CR_IN_CSR_BASE);
    printf("  FIFO_1TO4_PARAM_IN_BASE   = 0x%08x\n", (unsigned)FIFO_1TO4_PARAM_IN_BASE);
    printf("  FIFO_1TO4_PARAM_IN_CSR_BASE = 0x%08x\n", (unsigned)FIFO_1TO4_PARAM_IN_CSR_BASE);
    printf("  FIFO_1TO5_PARAM_IN_BASE   = 0x%08x\n", (unsigned)FIFO_1TO5_PARAM_IN_BASE);
    printf("  FIFO_1TO5_PARAM_IN_CSR_BASE = 0x%08x\n", (unsigned)FIFO_1TO5_PARAM_IN_CSR_BASE);
    printf("  FIFO_1TO6_FNAME_IN_BASE   = 0x%08x\n", (unsigned)FIFO_1TO6_FNAME_IN_BASE);
    printf("  FIFO_1TO6_FNAME_IN_CSR_BASE = 0x%08x\n", (unsigned)FIFO_1TO6_FNAME_IN_CSR_BASE);
    printf("\nInitial FIFO status:\n");
    log_fifo_csr("Y_IN ",   FIFO_1TO2_Y_IN_CSR_BASE);
    log_fifo_csr("Cb_IN",   FIFO_1TO2_CB_IN_CSR_BASE);
    log_fifo_csr("Cr_IN",   FIFO_1TO2_CR_IN_CSR_BASE);
    log_fifo_csr("1to4_IN", FIFO_1TO4_PARAM_IN_CSR_BASE);
    log_fifo_csr("1to5_IN", FIFO_1TO5_PARAM_IN_CSR_BASE);
    log_fifo_csr("1to6_IN", FIFO_1TO6_FNAME_IN_CSR_BASE);
    printf("\n");
